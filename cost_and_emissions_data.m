...
e_rideshare = 177.85;  %emissions released per person riding uber or lyft (2 passengers minimus so divide avg emissions/pmt driving car by 2
e_car = 355.7;  %avg emissions/pmt driving car
...
e_hybrid = 210.83;
e_EV = 127.14;
e_scoot = 86.86; %emissions for riding scooter = 0, emissions for charging/repair 86.86 gCO2/passenger-mile

Y = [e_buson e_spinscoot e_rideshare e_car e_truck e_busoff e_ebike e_moped e_mcycle e_hybrid e_EV e_scoot];
X = categorical({'Bus on-peak','Spin e-Scooter','Rideshare','Personal Car', 'Personal Truck', 'Bus off-peak', 'e-bike','Moped', 'Motorcycle', 'Hybrid Vehicle','Electric Vehicle', 'e-scooter (other)'});
X = reordercats(X,{'Spin e-Scooter','e-bike','Bus on-peak','e-scooter (other)','Moped','Electric Vehicle', 'Motorcycle','Rideshare','Hybrid Vehicle','Personal Car', 'Personal Truck', 'Bus off-peak',});
figure(2)
bar(X,Y)
